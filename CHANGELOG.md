...
- **Homepage: accentuate the no-risk guarantee** (2026-06-16, B1): on the Phase 0 process card the "if it isn't valuable, you don't pay" line was buried at the end of the body copy. Split it into its own `.ax-process__guarantee` element (`homepage.html` + `HomeSections.css`), **pinned to the bottom of the card** (`margin-top:auto`) with a cyan-light top hairline, a small filled-cyan ✓ badge, and a bold "Risk-free." lead in `--action-primary`. Noticeable and on-brand without a garish song-and-dance (Andy). All tokens — no hardcoded hex (Check #7 green).

- **Homepage teaser: strip redundant CTAs** (2026-06-16, B1): removed the floating "Explore what we do →" button below the three-way-in grid (Andy: "this button is odd" — competed with the tiles) and the per-tile CTA labels ("Company enablement →" etc.) in `WhatWeDoTeaser.html`. The whole tile is already a link, so it now routes directly to each subsection (`/what-we-do/company-enablement/`, `/leadership-ai-coaching/`, `/talks-events/`) — one click target per card, no competing CTAs. The `/what-we-do/` hub remains reachable via the nav. Verified via DOM on preview (deep-scroll screenshot blank locally — eyes-on on Netlify).
