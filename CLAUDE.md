...
**Last session:** 2026-06-14 — **Offer Canon established — single source of truth for what AX sells** ([#57](https://github.com/andy-carroll/accelerator-x-website/issues/57)). Root-caused #26: the offer was defined in 3 contradictory places. Adopted the recovered design intent (the **"Two Doors" model**), locked **all 6 founder decisions live** (names, pricing, workshop retired, proof/ROI, guarantee, events). Created `docs/business-context/offer-canon.md` (v0.3) + `content/data/offerings.json`; archived the 2 legacy offer docs; flagged stale specs/PRDs (dossier, landing-page-copy, 2 page-specs) with pointers. A **4-persona adversarial pass** produced a 22-question FAQ/objection bank + revisions list (proof gap = #1 ship-blocker; ROI multiple removed as unmeasured). **Also:** production "outage" root-caused to **Sky Broadband Shield** (ISP filter), NOT DNS — site was healthy globally throughout; corrected the prior "Hostinger parking IP" misdiagnosis (`90.207.238.183` = SKY-BROADBAND); reputation sweep all clean. See `dns-hosting.md` incident log. **Still not done:** manual v2 preview walkthrough.
