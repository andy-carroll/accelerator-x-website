...
**Last session:** 2026-06-13 — **Production HTTPS outage diagnosed and resolved** (DNS reverted to Hostinger parking NS ~2026-06-11; migrated to Netlify-managed DNS `dns1-4.p03.nsone.net`; full zone transferred, SSL re-provisioning in progress). **Site-config token system introduced** (`scripts/site-config.js`): founder LinkedIn URLs corrected (both had wrong slugs) + all founder emails, quiz URL, company LinkedIn tokenised as `{{site:KEY}}` across 8 templates; check #9 added to `scripts/check.js` enforces no hardcoded values. DNS/hosting reference doc created: `docs/tech-architecture/dns-hosting.md`. Manual preview walkthrough **not yet done** — session consumed by production incident.
...
**Known issues (live):** HTTPS was down ~2 days (Netlify DNS migration in progress, propagating at session close) · LinkedIn Post Inspector "No author found" (likely cache, low priority) · hero imagery still interim stills.
