...
**Last session:** 2026-08-05 — four sessions' worth of work landed today. (1) SEO indexation batch (#81 + session-executable parts of #40/#43): sitemap coverage fixed (20 → 30 URLs), FAQPage JSON-LD auto-derived onto 8 of 14 articles, insights-index canonical/title/description fixed, Organization JSON-LD enriched, `llms.txt` rewritten, `/quiz/*` 301s restored. (2) Compute-economics blog series (ax-agent-hub #1003): four new articles published (`015`–`018`), each with an inline SVG diagram, independently adversarially verified against the source memo's redaction gate — found and fixed a backwards `next_article` chain before shipping. (3) Caught after Andy flagged it: three of the four posts published with future dates (the every-2-days cadence was implemented as forward dates on a static site with no scheduling mechanism) — backdated to a real scatter before today; all 14 pre-existing articles checked, none affected. (4) **The AI readiness scorecard has never been promoted through content — fixed** (#94): `lead_magnet_cta` existed in every article's frontmatter and was read into the build's token map, but was never referenced in `_templates/article.html` — it rendered on no page, ever; its value ("5,000+ Founders and CTOs...") was also false but moot, since nobody had seen it. Added a real CTA card to every article, routed through `{{site:QUIZ_URL}}`, with honest copy about the scorecard itself. **Founder actions still open from #81: verify the domain in Google Search Console, submit `sitemap.xml`, request indexing (~10 min).** (5) **That click-through immediately hit a NordVPN "Threat Protection Pro" phishing block on `quiz.accelerator-x.ai`** — the third occurrence of this domain family's known false-positive pattern (see `docs/tech-architecture/dns-hosting.md` incident log), and the first on the quiz subdomain specifically. Checked independently rather than assumed: valid cert, clean Vercel headers, 18-day-stable cached content, legitimate page content, no security-researcher coverage anywhere, and Google Safe Browsing showing "no available data" (ruling out the block being Google-sourced). Andy submitted the miscategorisation report to NordVPN same-day — the first time this repeat incident's own deferred lesson has actually been executed rather than logged and left. Awaiting NordVPN's response.
