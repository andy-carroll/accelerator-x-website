...
**Last session:** 2026-07-06 — **Session-protocol continuity fixed properly + #82 opened as the next engineering priority.** Post-wrap continuation of the 2026-07-04 session surfaced that this repo's native session scripts and the portable `ax-skill-ops` `/session-start`//`session-end` skills had diverged into incompatible duplicates with real gaps: (1) the portable skills couldn't find this repo's handoff notes at all (no config block; wrong default dir) — fixed by adding the `## Session protocol — repo config` block below, mapped to reality; (2) the native brief silently dropped everything but the Summary paragraph from past session notes (h2-heading collision between the notes template and the log format) — fixed in both `session-end.js` (demote embedded headings) and `session-start.js` (capture the full section, old and new logs); (3) an earlier fix: `session-start.js` showed a "Define priorities" placeholder instead of the real Next Session Priorities whenever the section wasn't a numbered list (`a12fc70`). Also preserved the 2026-07-04 architecture-review scorecard as `docs/tech-architecture/architecture-review-2026-07-04.md` (7.0 → ~8.0–8.2/10) after finding it existed only in chat, and opened **#82** (child of #50) for the remaining reconciliation: retire-vs-keep the native scripts, port the independent-review gate. All committed + pushed; checks green.
...
This repo runs its **native** protocol first — `npm run session-start` / `npm run session-end` (config: `.session-protocol.json`, procedure: `.claude/rules/session.md`). The block below additionally makes the portable `ax-skill-ops` `/session-start` / `/session-end` skills work here by mapping their contract onto this repo's reality, so both routes find the same handoff notes and enforce the same gates. The two systems are being reconciled in [#82](https://github.com/andy-carroll/accelerator-x-website/issues/82) — until then, keep this block and `.session-protocol.json` in sync when either changes.
...
session_protocol:
  ...
  review:
    enabled: true
    command: "/code-review"                # independent fresh-eyes pass; native protocol lacks this gate (#82)
  stakeholder_summary:
    enabled: true
  ...
  extra_orientation:
    - "Native protocol is primary: npm run session-start / session-end (see .claude/rules/session.md); this block exists so the portable skills also work — reconciliation tracked in #82"
