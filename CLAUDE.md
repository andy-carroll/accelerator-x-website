...
1. **Adversarial review/QA sweep of the Phase 5 surfaces (recommended next).** Phase 5 is fully built (#57 closed) but reviewed only by build/check + spot screenshots — the local preview can't serve subpaths, so the offering pages, `/faq/` and Talks haven't had eyes-on layout/responsive checks. Run a multi-agent review (Canon-fidelity, truth/no-fabrication, a11y, broken links, responsive at 375/768/1280) across `_templates/offerings/*`, `TwoDoors`, `/faq/`, `talks-events`, home, `what-we-do`, `how-we-work`; verify findings adversarially before fixing. Then walk the **Netlify branch preview** (which resolves directory indexes) on desktop + 375px.
