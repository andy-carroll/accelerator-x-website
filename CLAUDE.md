...
**Start here (next session): [#83 — session-end gate-block dedupe + extractor edge-case tests](https://github.com/andy-carroll/accelerator-x-website/issues/83)** (~30-min warm-up; sequencing agreed with Andy, 2026-07-06) — then pick up #79 (marker-injection consolidation + atomic build writes), #80 (CHANGELOG rotation), or #51/#52/#54 (operating-model children); nothing engineering-side blocks the flip. Context: #82 CLOSED (2026-07-06) — protocol layering decided (portable skills = practice, native scripts = this repo's deterministic gates); **every close now requires an independent `/code-review` (or fresh-context subagent) pass first** — `session-end:write` blocks without a recorded `## Review` outcome in session-notes.md. The export follow-up (first-class gate-layer contract keys) is tracked as [ax-agent-hub#551](https://github.com/andy-carroll/ax-agent-hub/issues/551) — build it there only after this repo's gate survives 1–2 more real closes. The only step to the v2 flip (B10) remains **Andy/Toby's content-honesty walk of B1–B6** — not an engineering task.
