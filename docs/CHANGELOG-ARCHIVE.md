...
> Rotation policy decided in [#80](https://github.com/acc-x/accelerator-x-website/issues/80),
> 2026-08-22: `CHANGELOG.md`'s root file holds `[Unreleased]` plus current, actively-relevant
> history. Sections that are fully closed out — a tagged pre-cutover release, or (going forward)
> a completed calendar year once one exists — move here to keep the root file from becoming an
> ever-growing agent-context tax. Nothing is deleted; this file is the permanent record.
>
> **Moved here 2026-08-22:** the three pre-v2 versioned releases below (`0.1.0`–`1.0.0`,
> Feb–Mar 2026) — the original v1 site's build history, entirely superseded by the v2 rebuild
> (shipped 2026-07-14) and untouched since. `CHANGELOG.md`'s `[Unreleased]` section (the
> continuously-growing v2-era history) was deliberately left in place — splitting it further
> requires editorial judgment about where to cut, which belongs to a future pass, not this one.
