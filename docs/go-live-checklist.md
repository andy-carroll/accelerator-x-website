...
- [x] `main` branch matches the launch candidate (no unmerged feature work)
- [ ] `npm install` succeeds without warnings
- [x] `npm run build` completes locally with no errors
- [x] `scripts/test-site.js` passes (hero media + CTAs render) — 2026-03-20
- [x] `scripts/img-audit.sh` passes (responsive images + attributes) — 2026-03-20
- [x] Netlify build command — N/A: build artefacts are pre-built locally and committed; Netlify serves repo root as-is. Documented in `netlify.toml`.
...
- [x] `<title>` + `<meta description>` finalised — title expanded to "Accelerator X — AI Transformation for Business Leaders"; meta description in place
- [x] Canonical URL points to production domain (`https://accelerator-x.ai/`)
- [ ] Open Graph + Twitter cards tested (Meta debugger / Card validator) — manual: Andy to run
- [ ] JSON-LD validated via Rich Results Test (Organization, ProfessionalService, WebPage) — manual: Andy to run
- [x] `sitemap.xml` + `robots.txt` generated and deployed — auto-generated by `npm run build` (8 URLs); `robots.txt` in repo root
- [ ] Google Search Console property prepared for `accelerator-x.ai` — manual: Andy to set up
