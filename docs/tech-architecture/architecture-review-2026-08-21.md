...
| Testing & CI | 10% | 6.5 | ≈ held (7) — money-path test coverage grew markedly since baseline (16 cases → a 44-case suite incl. build-script tests, all green); but both merge gates had confirmed defects (#105 false-PASS, #107 world-state gating — **both fixed this session**); check.js itself untested |
